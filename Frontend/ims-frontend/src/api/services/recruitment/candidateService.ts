...
export const candidateService = {
    registerCandidate: async (data: RegisterCandidateCommand): Promise<FindCandidateByIdQueryResponse> => {
        const response = await RecruinmentApi.post<FindCandidateByIdQueryResponse>('/candidates/register', data);
        return response.data;
    },

    acceptToInternship: async (candidateId: string): Promise<void> => {
        await RecruinmentApi.put(`/candidates/accept-to-internship`, { candidateId });
    },

    updateCvLink: async (data: UpdateCvLinkCommand): Promise<FindCandidateByIdQueryResponse> => {
        const response = await RecruinmentApi.put<FindCandidateByIdQueryResponse>('/candidates/update-cv-link', data);
        return response.data;
    },

    getCandidateByEmail: async (email: string): Promise<FindCandidateByEmailQueryResponse> => {
        const response = await RecruinmentApi.get<FindCandidateByEmailQueryResponse>(`/candidates/by-email/${email}`);
        return response.data;
    },

    getCandidateById: async (id: string): Promise<FindCandidateByIdQueryResponse> => {
        const response = await RecruinmentApi.get<FindCandidateByIdQueryResponse>(`/candidates/${id}`);
        return response.data;
    },

    getAllCandidates: async (pageNumber = 1, pageSize = 10): Promise<GetAllCandidatesQueryResponse> => {
        const response = await RecruinmentApi.get<GetAllCandidatesQueryResponse>('/candidates/get-all', {
